...
public class SensorTagData {
    ...
    public static double extractLED(BluetoothGattCharacteristic c) {
        Log.i(TAG, "Inside the Sensortag class");
        int value = shortUnsignedAtOffset(c, 0);
        Log.i(TAG, "Inside the sensortag class: after shortUnisignedOffsetfunction");
        return value * 1.0;
    }
    ...
    private static Integer shortUnsignedAtOffset(BluetoothGattCharacteristic c, int offset) {
        Log.i(TAG, "Entering the upper bound of the offset function");
        Integer lowerByte = c.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, offset);
        Integer upperByte = c.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, offset + 1); // Note: interpret MSB as unsigned.
        Log.i(TAG, "Exiting the lower bound of the offset function");
        Log.i(TAG, "Exit value is: " + lowerByte + " and " + upperByte);
        return lowerByte;
    }
}